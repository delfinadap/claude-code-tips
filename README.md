...
Here is a real example: [TESTING.md](TESTING.md) from this repo. It started with the review of one external PR, where Claude Code reproduced the bug on a real Windows runner in GitHub Actions, verified the fix, and summarized everything it tested. Now it's a running log that we keep updating with each review. Tracking it in git this way gives you a record of how everything has been tested. If you want to know if something works as intended, then you can simply look at this document.
